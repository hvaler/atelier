...
| **Two-Plane Critique Model** | ✅ **100% Complete & Validated** | Plane A (OpenCV measured) + Plane B (Studio qualitative rubric) strictly separated. Calibrated against published art curricula from **seven programmes in five countries** — see **[docs/PEDAGOGY.md](docs/PEDAGOGY.md)** for the criteria comparison, the consolidated vocabulary, the cross-institutional comparison and the sourced exercise progression. |
...
| **Cylinders & ellipses** | ⏳ *Not measured* | BYU-Idaho's published rubric scores ellipse shape and major/minor axis; Atelier has no ellipse detection. A documented rung of the curriculum ladder ([PEDAGOGY §4](docs/PEDAGOGY.md)) that the engine cannot yet assess, so the agent should not prescribe it. |

...
>  
> *Switching the level from basic to advanced shifts the vocabulary to the terms of the discipline: line of horizon ($LH$), ground line ($LT$), $F_1/F_2$ convergence, true magnitude. The profile is a difficulty setting, not a person — it decides the register of the critique and nothing else. In the **Progression tab** the angular error across sessions is read from Firestore, alongside the weekly practice plan generated by Cloud Scheduler."*
